...
describe 'Bitcoin::Protocol::Block' do
  ...
  it "should parse merkle block" do
    ...
    block.hashes.map(&:hth).should == [
      "4e99f644670144736982bfae805a176c2caa5f9fd4e4a728e82eca2d1fa3312c",
      "aa8ff248d2e487bbc589c03d127ae3272bf4a7717fd3c5011cfb63249478b2d1" ]
    block.flags.should == [7]
  end

  it "should parse merkle block from full block data" do
    block = Bitcoin::P::MerkleBlock.new(@blocks['0'])
    block.hash.should == "00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048"
    block.tx_count.should == 257
    block.hashes.should == []
  ...
  it "should convert block to merkle block" do
    ...
    merkle_block.hashes.map(&:hth).should == block.tx.map(&:hash)
  end

  it "should serialize with disk attributes" do
    block = Bitcoin::P::MerkleBlock.new(@data)
    block.depth, block.chain, block.work = 1, 2, 34567890
    b = Bitcoin::P::MerkleBlock.from_disk(block.to_disk)
    b.hash.should == block.hash
    b.hashes.should == block.hashes
    b.depth.should == block.depth
    b.chain.should == block.chain
    b.work.should == block.work
  end
