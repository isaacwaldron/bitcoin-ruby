...
module Bitcoin::Storage::Backends
  ...
  class SequelStore < StoreBase
    ...
    def store_tx(tx, validate = true)
      ...
      # TODO optimize
      tx.out.each {|o|
        relevant = true  if (Bitcoin::Script.new(o.pk_script).get_addresses.map {|a| Bitcoin.hash160_from_address(a) } & watched_addrs).any?
      }
      tx.in.each {|i|
        relevant = true  if get_tx(i.prev_out.reverse_hth)
      }
